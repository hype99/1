if (confirm("Join For More Links!!")) {
        ...
        async function initApp() {
            ...
            try {
                ...
                // 2. Check for Iframe URL (New Category)
                if (data.ifurl && data.ifurl.trim() !== "") {
                    loadIframe(data.ifurl);
                    return;
                }

                // 3. Check for Shaka/Standard Player URL
                const url = data.url;
                if (!url) {
                    window.location.href = CONFIG.tg;
                    return;
                ...
                // If none of the above, load Shaka
                loadShaka(data);
        ...
        function loadIframe(url) {
            ...
            if(pc) pc.classList.add('hidden'); // Hide Shaka container
        ...
        async function loadShaka(data) {
            ...
            iframe.classList.add('hidden'); // Ensure iframe is hidden
